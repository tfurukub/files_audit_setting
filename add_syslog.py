##############################################################################
#
#  Script: Set syslog server information  via REST API (v3)
#  Author: Takeo Furukubo
#  Description: Set syslog server information
#  Language: Python2.7
#
##############################################################################
...
if __name__ == "__main__":
    try:
        ...
        args = sys.argv
        if len(args) < 4:
            print("Please specify <syslog IP>,<port number>,<server name>")
        else:
            status,response = rest_api.set_syslog_server(args[1],args[2],args[3])
            if status != 202:
                print("Failed to add {}, Reason = {}").format(args[3],response)
